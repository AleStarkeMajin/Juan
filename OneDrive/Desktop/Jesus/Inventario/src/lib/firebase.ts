...
signInAnonymously(auth)
	...
	.catch((err) => console.debug('Anonymous sign-in failed or not enabled:', err && err.code ? err.code : err));
...
export const ensureSignedIn = async () => {
	...
	try {
		...
		console.debug('ensureSignedIn: anonymous sign-in failed', err && err.code ? err.code : err);
		return null;
	}
};
